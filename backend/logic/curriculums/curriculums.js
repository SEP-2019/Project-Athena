    ...
    try {
      ...
      for (let core in cores)
      {
        ...
        if(course_count){
          ...
          await connection.query("INSERT INTO curriculum_core_class (curriculum_name, course_code) VALUES(?, ?);", [
            name,
            core
          ]);
  
        } else {
          throw new Error("One of these courses does not yet exist.");
        }
      }
      for (let techComp in techComps)
      {
        ...
        if(course_count){
          ...
          await connection.query("INSERT INTO curriculum_tech_comp (curriculum_name, course_code) VALUES(?, ?);", [
            name,
            techComp
          ]);
      ...
      for (let comp in comps)
      {
        ...
        if(course_count){
          ...
          await connection.query("INSERT INTO curriculum_complementaries (curriculum_name, course_code) VALUES(?, ?);", [
            name,
            comp
          ]);
